...
data "consul_keys" "networking" {
  key {
    name = "networking"
    path = terraform.workspace == "default" ? "networking/configuration/globo-primary/net-info" : "networking/configuration/globo-primary/${terraform.workspace}/net-info"
  }
...
locals {
  ...
  common_tags = merge(jsondecode(data.consul_keys.networking.var.common-tags),
    {
